...
type internal UInt64ConverterSettings = {
    Optional: bool }
    with
    static member val Default = {
        UInt64ConverterSettings.Optional = false }

type internal UInt64Converter(converterSettings: UInt64ConverterSettings) =
    let dotnetType = typeof<uint64>
    let dataDotnetType = typeof<uint64>
    ...
    static member val Default = UInt64Converter(UInt64ConverterSettings.Default)
