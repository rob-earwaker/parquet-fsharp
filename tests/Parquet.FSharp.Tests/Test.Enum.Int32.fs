...
module ``serialize int32 enum`` =
    ...
    [<Theory>]
    [<InlineData(Enum.Value1)>]
    [<InlineData(Enum.Value2)>]
    [<InlineData(Enum.Value3)>]
    let ``value`` value =
...
module ``deserialize int32 enum from required int32`` =
    ...
    let ``undefined value`` value =
        ...
        let bytes = ParquetSerializer.Serialize(inputRecords)
        let outputRecords = ParquetSerializer.Deserialize<Output>(bytes)
        test <@ outputRecords = [| { Output.Field1 = enum<Enum> value } |] @>
...
module ``deserialize int32 enum from required int16`` =
    ...
    [<Theory>]
    [<InlineData((* inputValue *) Int16.MinValue, (* outputValue *) enum<Enum> -32768)>]
    [<InlineData((* inputValue *)            -1s, (* outputValue *) enum<Enum>     -1)>]
    [<InlineData((* inputValue *)             3s, (* outputValue *) enum<Enum>      3)>]
    [<InlineData((* inputValue *) Int16.MaxValue, (* outputValue *) enum<Enum>  32767)>]
...
module ``deserialize int32 enum from required int8`` =
    ...
    [<Theory>]
    [<InlineData((* inputValue *) SByte.MinValue, (* outputValue *) enum<Enum> -128)>]
    [<InlineData((* inputValue *)            -1y, (* outputValue *) enum<Enum>   -1)>]
    [<InlineData((* inputValue *)             3y, (* outputValue *) enum<Enum>    3)>]
    [<InlineData((* inputValue *) SByte.MaxValue, (* outputValue *) enum<Enum>  127)>]
    let ``undefined value`` inputValue outputValue =
        let inputRecords = [| { Input.Field1 = inputValue } |]
        let bytes = ParquetSerializer.Serialize(inputRecords)
        let outputRecords = ParquetSerializer.Deserialize<Output>(bytes)
...
module ``deserialize int32 enum from required uint16`` =
    ...
    [<Theory>]
    [<InlineData((* inputValue *)             3us, (* outputValue *) enum<Enum>     3)>]
    [<InlineData((* inputValue *) UInt16.MaxValue, (* outputValue *) enum<Enum> 65535)>]
    let ``undefined value`` inputValue outputValue =
        let inputRecords = [| { Input.Field1 = inputValue } |]
        let bytes = ParquetSerializer.Serialize(inputRecords)
        let outputRecords = ParquetSerializer.Deserialize<Output>(bytes)
...
module ``deserialize int32 enum from required uint8`` =
    ...
    [<Theory>]
    [<InlineData((* inputValue *)           3uy, (* outputValue *) enum<Enum>   3)>]
    [<InlineData((* inputValue *) Byte.MaxValue, (* outputValue *) enum<Enum> 255)>]
    let ``undefined value`` inputValue outputValue =
        let inputRecords = [| { Input.Field1 = inputValue } |]
        let bytes = ParquetSerializer.Serialize(inputRecords)
        let outputRecords = ParquetSerializer.Deserialize<Output>(bytes)
