...
module ``{ optional=false } serialize`` =
    type Input = { [<ParquetUInt16(Optional = false)>] Field1: uint16 }
    type Output = { Field1: uint16 }
...
module ``{ optional=true } serialize`` =
    type Input = { [<ParquetUInt16(Optional = true)>] Field1: uint16 }
...
module ``{ optional=false } deserialize`` =
    type Input = { Field1: uint16 }
    type Output = { [<ParquetUInt16(Optional = false)>] Field1: uint16 }
...
module ``{ optional=true } deserialize`` =
    ...
    type Output = { [<ParquetUInt16(Optional = true)>] Field1: uint16 }
        ...
        raisesWith<SerializationException>
            ...
            (fun exn ->
                <@ exn.Message =
                    ...
                    + $" non-nullable type '{typeof<uint16>}'" @>)
